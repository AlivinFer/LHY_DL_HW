...
if __name__ == '__main__':
    ...
    arrayTrainX, arrayTrainY, arrayValidX, arrayValidY = getTrainAndValidData(arrayTrainAllX=arrayTrainX,
                                                                              arrayTrainAllY=arrayTrainY,
                                                                              percentage=0.3)

    # calculate maximum likelihood estimator of mu and sigma
    ...
    # clip() 设置列表的最小最大值
    arrayMu1 /= np.clip(intCount1, 1, intTrainSize)
    arrayMu2 /= np.clip(intCount2, 1, intTrainSize)
    ...
    # 矩阵求逆(最好使用 pinv，即使矩阵不可逆也能求出参数)
    arrayCovarianceInverse = np.linalg.pinv(arrayCovariance)

    # validation
    arrayW = np.dot(np.transpose(arrayMu1 - arrayMu2), arrayCovarianceInverse)
    arrayB = -0.5 * np.dot(np.dot(np.transpose(arrayMu1), arrayCovarianceInverse), arrayMu1) + 0.5 * np.dot(
        np.dot(np.transpose(arrayMu2), arrayCovarianceInverse), arrayMu2) + np.log(float(intCount1 / intCount2))

    z = np.dot(arrayW, np.transpose(arrayValidX)) + arrayB
    s = getSigmoidValue(z)
    result = ((np.around(s)) == np.squeeze(arrayValidY))
    print("Valid Accuracy:{} ".format(float(result.sum()) / len(arrayValidY)))
